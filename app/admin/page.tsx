...
export default function AdminPanel() {
    ...
    await updatePhone({
      variables: {
        ...
        input: {
          ...
          price: Number(editPhone.price),
          storageGb: Number(editPhone.storageGb),
          ramGb: Number(editPhone.ramGb),
  ...
  const filteredPhones =
    data.phones.filter((p: any) =>
      `${p.brand} ${p.model}`.toLowerCase().includes(search.toLowerCase())
    );

  return (
    <div style={{ minHeight: "100vh", background: "#f2f6fc", padding: 30, fontFamily: "Arial" }}>
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        ...
        <div style={{
          ...
          {newPhone.imageUrl && <img src={newPhone.imageUrl} width={120} style={{ marginTop: 10, borderRadius: 8 }} />}

          <button onClick={handleAdd} style={{ marginTop: 15, padding: "10px 18px", borderRadius: 10, border: "none", background: "#007aff", color: "#fff", cursor: "pointer" }}>
            Add Phone
          </button>
        </div>
        {/* ===== END ADD PHONE CARD ===== */}
        ...
        {/* SEARCH */}
        <input placeholder="Search phone..." value={search} onChange={e => setSearch(e.target.value)} style={{ padding: 10, marginBottom: 20, width: 300, borderRadius: 10, border: "1px solid #ccc" }} />
        ...
        <table style={{ width: "100%", borderCollapse: "collapse", background: "#fff", borderRadius: 10, overflow: "hidden", boxShadow: "0 5px 15px rgba(0,0,0,0.05)" }}>
          ...
          <tbody>
            {filteredPhones.map((phone: any) => (
              <tr key={phone.id}>
                <td style={{ border: "1px solid #ccc", textAlign: "center", padding: 8 }}>{phone.image_url && <img src={phone.image_url} width={50} />}</td>
                ...
                <td style={{ border: "1px solid #ccc", padding: 8 }}>
                  {retailData?.retailers.find((r: any) => r.id === phone.retailer_id)?.name}
                </td>
                <td style={{ border: "1px solid #ccc", padding: 8 }}>
                  {catData?.categories.find((c: any) => c.id === phone.category_id)?.name}
                </td>
                <td style={{ border: "1px solid #ccc", padding: 8, display: "flex", gap: 5, position: "relative", zIndex: 1 }}>
                  <button
                  ...
                  <button
                    onClick={() => setEditPhone(normalizePhone(phone))}
        ...
        {viewPhone && (
          ...
          >
            ...
            >
              ...
              {viewPhone.image_url && <img src={viewPhone.image_url} width={200} style={{ marginBottom: 15, borderRadius: 10 }} />}
              ...
              <button
                onClick={() => setViewPhone(null)}
                style={{ marginTop: 20, padding: 12, borderRadius: 10, border: "none", background: "#aaa", color: "#fff", cursor: "pointer", fontWeight: 600 }}
              >
                Close
              </button>
            </div>
        ...
        {editPhone && (
          ...
          >
            ...
            >
              ...
              {editPhone.imageUrl && <img src={editPhone.imageUrl} width={200} style={{ marginTop: 15, borderRadius: 10 }} />}
              ...
              <div style={{ display: "flex", gap: 15, marginTop: 20 }}>
                <button onClick={handleUpdate} style={{ flex: 1, padding: 12, borderRadius: 10, border: "none", background: "#007aff", color: "#fff", cursor: "pointer", fontWeight: 600 }}>
                  Save
                </button>
                <button onClick={() => setEditPhone(null)} style={{ flex: 1, padding: 12, borderRadius: 10, border: "none", background: "#aaa", color: "#fff", cursor: "pointer", fontWeight: 600 }}>
                  Cancel
                </button>
              </div>
            </div>
